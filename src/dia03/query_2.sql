select date('{date}') as dt_ref,
    ...
    from (
        select t2.seller_id,
                ...
                cast(julianday(date('{date}')) - julianday( max( t1.order_approved_at ) ) as int) as qtd_dias_ult_venda,
                ...
                sum( case when t1.order_delivered_customer_date > t1.order_estimated_delivery_date then 1 else 0 end ) / count(distinct t2.order_id) as prop_atraso,
                ...
                sum(t2.price) / count(distinct strftime('%m', t1.order_approved_at) ) as avg_vl_venda_mes_ativacao,
        ...
        left join(
            select t2.seller_id,
                    max(julianday(date('{date}')) - julianday(t1.order_approved_at)) as idade_base
            ...
            where t1.order_approved_at < date('{date}')
